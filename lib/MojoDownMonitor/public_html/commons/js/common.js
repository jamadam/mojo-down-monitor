...
$(document).ready(function() {
	...
	$("*[rel='external'], .external").bind('click.blankEmulate', function(){
		window.open($(this).attr("href"));
		return false;
	});
	
	$.currentTabManager('#buttons a').fire({
		classCurrent : 'enabled'
	});
	
	$.currentTabManager('.thirdMenu a').fire({
		classCurrent : 'enabled'
	});
	...
	function swapContent(global) {
		...
		cont.animate({top: "0",opacity: "1"}, 300, "bounce");
		loading.hide();
	}
});
